...
def _clang_time_trace_aspect_impl(target, ctx):
    ...
    for src in ctx.rule.files.srcs:
        if src.short_path.endswith('.h'):
          continue
        # Declare the trace output file
        trace_output_file = ctx.actions.declare_file(paths.basename(src.short_path) + ".json")
        output_files.append(trace_output_file)

        

        print(target,src,src.short_path,trace_output_file.path)
        ...
        ctx.actions.run(
            ...
            arguments = compiler_flags + ["-ftime-trace="+trace_output_file.path,"-ftime-trace-granularity=1", "-c",src.path],
        )
